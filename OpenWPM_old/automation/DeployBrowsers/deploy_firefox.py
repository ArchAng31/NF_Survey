...
def deploy_firefox(browser_params):
    ...
    if browser_params['pinning']:
        pin_loc = os.path.join(root_dir, 'firefox_extensions/pinutil_addon.xpi')
        fp.add_extension(extension=pin_loc)
        fp.set_preference("extensions.pin_util", "1.0")  # Avoid startup screen

    if browser_params['lastpass']:
        lastpass_loc = os.path.join(root_dir, 'firefox_extensions/lastpass.xpi')
        fp.add_extension(extension=lastpass_loc)
        fp.set_preference("extensions.lastpass", "3.1")  # Avoid startup screen
